...
var phscoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in phscoreDefaults)
    if (phscoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = phscoreDefaults[i];
